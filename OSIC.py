...
# Validation Lists
valid_prov = ["NL", "PE", "NS", "NB", "QC", "ON", "MB", "SK", "AB", "BC", "NV", "NW", "YK"]
...
# Street Address, mandatory input 
    while True:
...
# Province, mandatory input, compared to valid list of provinces
    while True:
...
# Postal Code, mandatory input, must be valid format as X0X 0X0 ------ADD REPLACE STATEMENTS 
    while True:
        post_code = input("Please enter code (L0L 0L0): ").upper()
        if post_code[3] != " ":
            print("Please include a space between postal code")
        elif len(post_code) != 7:
            print("Please enter postal code as: L0L 0L0")
        else:
            break
...
# Phone Number, mandatory input, 10 characters long
    while True:
        ...
        if len(phone_num) != 10:
            ...
            formatted_phone_num = formatted_phone_number = phone_num[:3] + "-" + phone_num[3:6] + "-" + phone_num[6:]
            break
...
# Receipt generation 
    ...
    print(f"Payment Frequency:       {payment_text:<7s}          Additional Car Cost:    {dsp_add_car_cost:>9}   ")
    print(f"                                          Basic Premium Total:   {dsp_basic_prem:>10}")
    print(f"                                          ---------------------------------")
    print(" " * 42, "Extra Costs".center(33))
    print(f"                                          Extra Liability Cost:  {dsp_ex_insurance_cost:>10}")
    print(f"                                          Glass Coverage Cost:   {dsp_glass_cov_cost:>10}")
    print(f"                                          Loaner Car Cost:       {dsp_loaner_car_cost:>10}")
